...
class JobController extends Controller
{
    ...
    {
        $this->middleware(['helper', 'verified'], ['except'=>['index', 'show', 'apply', 'allJobs', 'searchJobs']]);
    }

    public function index(){
        $jobs = Job::latest()->where('status', 1)->whereDate('deadline','>',date('Y-F-d'))->limit(10)->get();
        $categories = Category::with('jobs')->get();
//        $advisors = Advisor::all()->random(8);
    ...
    public function show($id, $job){
        ...
        $jobsBasedOnCategories = Job::latest()
            ...
            ->limit(6)
            ->get();
        $jobsBasedOnAdvisors = Job::latest()
        ...
        $jobsBasedOnPositions = Job::latest()
            ...
            ->limit(6)
            ->get();
        array_push($data, $jobsBasedOnCategories, $jobsBasedOnAdvisors, $jobsBasedOnPositions);
